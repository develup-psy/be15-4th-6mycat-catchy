...
<details>
  ...
  <div align="center">
    <img src="./asset/댓글, 답글 달기 및 취소.gif" />
  </div>
</details>
...
| 박성용 | 이번 DevOps 프로젝트에서 저는 Jenkins, Docker, DockerHub, Kubernetes, ArgoCD를 주된 기술 스택으로 사용해 CI/CD 파이프라인을 구축했습니다.
백엔드(Spring Boot)는 Gradle, 프론트엔드(Vue.js)는 Node.js 환경에서 빌드하고,
Docker 이미지로 빌드한 후 Jenkins Credentials를 활용해 DockerHub에 안전하게 푸시했습니다.
이후 Kubernetes manifests 저장소를 자동 갱신해 최신 이미지를 반영했으며, 초기에는 Jenkins에서 서버 bash로 argocd CLI를 호출해 직접 app sync, app wait로 ArgoCD를 제어했습니다.
그런데 프로젝트를 진행하면서 Jenkins와 ArgoCD의 역할 분리가 중요하다는 걸 깨달았습니다.
원래 Jenkins의 책임은 매니페스트 수정 및 깃허브 푸시까지이고,
그 이후의 쿠버네티스 동기화 및 자동화는 깃 레포 변경을 감지한 ArgoCD가 맡아야 하는 부분이었습니다.
이 인식 전환 덕분에 파이프라인의 책임 경계를 명확히 나누고 보다 깔끔한 GitOps 형태로 개선할 수 있었습니다.
또한 배포 후 단일 진입점으로 Ingress를 설정하는 과정에서 문제가 발생했습니다.
Presigned URL을 통한 S3 업로드에서 Ingress 프록시를 거치면서 서명이 깨지는 이슈가 있었는데,
이때 특정 요청에 대해서는 프록시를 통하지 않고 직접 S3 API로 요청을 우회하는 방식을 적용해 문제를 해결했습니다.
이 과정에서 네트워크 레이어, 프록시 동작 방식, HTTP 헤더 전파 등에 대한 이해도를 높일 수 있었습니다.
사실 이번 프로젝트에서 원래 목표는 **ELK(Elasticsearch, Logstash, Kibana)**로 로그 수집,
Grafana + Prometheus로 메트릭 모니터링, Kafka 같은 이벤트 큐 연동,
그리고 **클라우드 환경(RDS, S3, CloudFront 등)**으로 완전한 상용 배포까지 확장하는 것이었습니다.
하지만 로컬 자원 부족, 시간 제약, 보안 설정 난이도로 인해 일부 목표는 다음으로 미루게 되었습니다.
그래도 Jenkins와 ArgoCD의 연계, 쿠버네티스의 실제 PVC, ConfigMap, RBAC 설정,
프록시 우회 같은 실전적인 문제 해결 경험은 큰 성장의 발판이 되었습니다.
다음에는 Helm Chart를 활용한 배포, 클라우드 인프라 연동, GitOps 완전 자동화,
